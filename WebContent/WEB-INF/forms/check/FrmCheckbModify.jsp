...
<body>
	...
	<div>
		<form action="FrmCheckbUpdate" method="post">
		商品编号<input type="text" name="code" value="${checkb.items.Code_}" readonly="readonly"/> <br />
		 ...
		 <input type="hidden" name="tbNo" value="${checkb.items.TBNo_}" />
		    <input type="submit" value="保存" />    <a href="FrmCheckbDelete?code=${checkb.items.Code_ }&num=${checkb.items.NewStock_}&tbNo=${checkb.items.TBNo_}" >删除</a>
		</form>
	</div>
	<div>${message }</div>
	<div style="padding: 0.5em;">
		<a href="FrmCheckList?tbNo=${checkb.items.TBNo_}">返回</a>
	</div>
